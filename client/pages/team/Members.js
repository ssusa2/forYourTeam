...
function Members({ test, OneMember }) {
	const { id, name, image, role } = test;
	const isOne = OneMember == 1;
	console.log(isOne);
	return (
		<>
			<div>
				<div className={isOne ? 'xl:flex xl:relative' : ''}>
					<img
						className={
							isOne ? ' w-full xl:w-1/2 ' : 'object-cover w-full max-h-72	'
						}
						src={image}
					/>
					<div className={isOne ? 'xl:pl-10 pt-2' : 'base-m'}>
						<p className='w-full'>
